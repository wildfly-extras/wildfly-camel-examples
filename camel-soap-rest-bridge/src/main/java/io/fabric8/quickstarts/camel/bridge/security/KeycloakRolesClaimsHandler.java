...
public class KeycloakRolesClaimsHandler implements ClaimsHandler {
    ...
    public ProcessedClaimCollection retrieveClaimValues(ClaimCollection claims, ClaimsParameters parameters) {
        String userName = parameters.getPrincipal().getName();
        StringTokenizer tokenizer = new StringTokenizer(userName, ";", false);
        String tokenUser = tokenizer.nextToken();
        ...
        if (claims != null && claims.size() > 0) {
            ...
            for (Claim requestClaim : claims) {
                ...
                if (ROLE.equals(requestClaim.getClaimType())) {
                    ...
                    Keycloak keyCloak = KeycloakBuilder.builder().serverUrl(address).realm(realm)
                        .username(tokenUser).password(adminPassword).clientId("admin-cli")
                        .authorization(token)
                    ...
                    List<UserRepresentation> users = keyCloak.realm(realm).users().search(tokenUser);
                    if (users != null) {
